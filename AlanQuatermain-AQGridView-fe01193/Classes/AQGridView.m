...
{
    //init an AQGridViewData to hold gridView states;
    ...
    //[self setShowsVerticalScrollIndicator:NO];//Ttron
}
...
//the actual serial No. from a cell, oppsite to method:visibleCellListIndexForItemIndex
        ...
        @try
        {
                ...
                ([newVisibleIndices countOfIndexesInRange: _visibleIndices] != _visibleIndices.length) )
            {
                ...
                if ( [removedIndices count] != 0 )
                {
                    ...
                    [removedCells makeObjectsPerformSelector: @selector(removeFromSuperview)];//@_@
                ...
                _visibleIndices.location = [newVisibleIndices firstIndex];//@_@
...
- (void) layoutAllCells
{
    NSLog(@"layoutCellsInVisibleCellRange");
	[self sortVisibleCellList];
